...
const ChooseToUseTemplate = () => {
  ...
  return (
    <>
      <div className="fixed size-10 sm:size-11 lg:size-12 bg-blue-700 top-24 z-40 left-2 shadow-lg shadow-gray-900 bg-opacity-80 backdrop:blur-3xl lg:left-4 rounded-full flex items-center justify-center text-white hover:bg-opacity-100"><MoreVertical onClick={handleSeeMore} className="scale-50 active:scale-75 hover:cursor-pointer w-full h-full transition-all duration-300"/></div>
      {/* small screen dropdown */}
      {isOpen && (
      <div className={`z-50 sm:hidden fixed inset-0 bg-black bg-opacity-85 backdrop:blur-3xl`}>
        ...
        <div className={` ${isOpen ? "h-fit" : "h-0"} transition-all duration-300 w-full fixed bottom-0 p-6 rounded-t-3xl bg-white flex flex-col`}>
          <div className="flex flex-col gap-2 text-center">
            <div className="h-[8px] w-20 rounded-full bg-black m-auto -translate-y-4"/>
